...
def testing(model, ile_treningu, model_trenujacy):
    ...
    for game_iterator in range(400):
        ...
        while done[0] == False:
            action = get_action(model, game.getArr(), 0)  # 0 to epsilon, nie zmieniac do testowania
            if game.getArr()[0][action[0]] != 0:  # jezeli agent wybral zly ruch to przegrywa
                przegrane += 1
                invalids += 1
                break
            game.set_board_state(make_move(game.getArr(), action[0], 1))
            ...
            if done[0] == True:
                if 'Player 2' in done[1]:
                    ...
                    remisy += 1
                break
            random_move = returnMinMaxValue(game, model_trenujacy)
            game.set_board_state(make_move(game.getArr(), random_move, 2))
            done = check_if_done(game.getArr())
            if done[0] == True:
                if 'Player 2' in done[1]:
                    przegrane += 1
    ...
    result_string += "Bledne ruchy: " + str(invalids) + " Pionowe: " + str(v) + " Poziome: " + str(
        h) + " Przekatne: " + str(d) + "\n\n"
...
for i_episode in range(already_itered, 40000):
    ...
    while True:
        ...
        board_before_move = game.getArr().copy()
        arr, overflow = playingStep(game.getArr(), action, 1)
        game.set_board_state(arr)
